...
class ManiSkillVectorEnv(VectorEnv):
    ...
    def step(
        ...
        self.returns += rew # infos["episode"]["r"] = returns 指的是num_step步下来总的reward和
        ...
        if self.handle_truncations and self.max_episode_steps is not None:
            truncations: torch.Tensor = (
                self.base_env.elapsed_steps >= self.max_episode_steps # truncations一直是zero直到step达到max_episode_steps
            )
        if isinstance(terminations, bool):
            terminations = torch.tensor([terminations], device=self.device)
        if self.ignore_terminations:
            terminations[:] = False
        dones = torch.logical_or(terminations, truncations) # terminations:即task中定义的success判定结果， truncations:即step是否已超过num_step
        infos[
            "real_next_obs"
        ] = obs  # not part of standard API but makes some RL code slightly less complicated
        if dones.any(): # 判断是否有一个返回已完成任务，如有，返回的info字典中加入final_info项和下划线前缀的mask
            infos["episode"]["r"] = self.returns.clone()
            final_obs = obs
            env_idx = torch.arange(0, self.num_envs, device=self.device)[dones]
            obs, _ = self.reset(options=dict(env_idx=env_idx)) # 每个epoch会跑num_step步，这些会记录在env.elapsed_steps中但是一个epoch结束不会重置，只有当elapsed_steps达到max_eps_steps时才会重置，所以比如设置了每个epoch的num_step为20，但是max_eps_steps为50，就会在第三个epoch的第9个epoch处才出现truncation，然后再在此处reset整个env
